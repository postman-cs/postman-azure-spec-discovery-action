...
export class AppServiceProvider implements SpecProvider {
  ...
  public async listCandidates(): Promise<SpecCandidate[]> {
    ...
    for (const site of sites) {
      ...
      let apiDefinitionUrl = (site.apiDefinitionUrl ?? '').trim();
      ...
      if (this.options.runtimeClient) {
        const runtime = await this.options.runtimeClient.getSiteRuntimeConfig(site.resourceGroup, site.name);
        this.runtimeCache.set(armId, runtime);
      ...
      if (!apiDefinitionUrl && !apiSpecPath) continue;
      ...
      candidates.push({
        ...
        supported,
        evidence: [
          ...(apiDefinitionUrl ? [`App Service site ${site.name} declares an API definition URL`] : []),
          ...(apiSpecPath
            ? [`App Service site ${site.name} declares aiIntegration.ApiSpecPath=${apiSpecPath}`]
            : []),
