...
export class AccessTokenProvider {
  ...
  private async mintWithRetry(): Promise<string> {
    ...
    try {
      ...
      const original = maskPmakDiagnostic(error instanceof Error ? error.message : String(error), [this.apiKey, this.token]);
      if (error instanceof MintError && (error.status === 401 || error.status === 403)) {
        ...
        throw new MintError(formatRejectedMint(original, diagnosis), true, error.status);
      }
      throw new MintError(original, error instanceof MintError ? error.permanent : false, error instanceof MintError ? error.status : undefined);
    }
    this.token = token;
    this.onToken?.(token);
    return token;
