...
Gateway-tag repo resolution: per-service-repo automatic discovery via ownership tags (Fox hub concept applied at the repo level).
...
- Fox-style `GithubOrg`/`GithubRepo` tag pairs compose to `<org>/<repo>` and are select-grade beside canonical `postman:repo` — a gateway tagged with its owning repo resolves automatically for that repo's workflow run.
