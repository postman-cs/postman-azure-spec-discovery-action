...
- **HTTP** APIs export OpenAPI JSON. **SOAP** APIs use the same ARM export protocol with `wsdl-link` and write native `service.wsdl`. **GraphQL** APIs read the `graphql` schema (or the first GraphQL content type) through the Reader GET schema surface and write native `schema.graphql`; derivation also emits a deliberately partial OpenAPI 3.0.3 `/graphql` POST shell. **gRPC** APIs export authoritative native protobuf only when the documented APIM API Schema List/Get route (`api-version=2024-05-01`) returns a schema with `contentType` `text/protobuf`; when that schema is absent, the API stays visible as unsupported for manual review. WebSocket and OData APIs stay visible as unsupported candidates for manual review.
