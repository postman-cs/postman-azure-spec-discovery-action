...
// - Service-level postman:repo + GithubOrg/GithubRepo tags are inherited across
//   multiple APIs and must NOT select alone (narrow/unresolved without path evidence).
// - Canonical harness case path-selects payments-live; split-tag case path-selects orders-live.
...
var githubOrg = length(repoParts) > 0 ? repoParts[0] : 'postman-cs'
var githubRepo = length(repoParts) > 1 ? repoParts[1] : 'postman-azure-spec-discovery-action'
