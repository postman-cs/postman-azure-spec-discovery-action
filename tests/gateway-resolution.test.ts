...
describe('R1 host+path and tag selection', () => {
  ...
  it('R1-TAG-001: unique Fox tag selects; inherited service tag across two APIs narrows only', async () => {
    const fox = await runNarrowingPipeline(
      {
        repoSlug: 'org/payments',
        serviceHints: [],
    ...
    expect(fox?.mode).toBe('select');
    expect(fox?.apiIds).toEqual([arm('payments')]);
